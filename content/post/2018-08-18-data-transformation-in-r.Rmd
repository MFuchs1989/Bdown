...
# Introduction
In dem nachfolgenden Post geht es um das Thema Data Transformation in R. Je nach Blickwinkel fällt dieses Thema in den Bereich Data Data Wrangling oder Data Exploration.
...
Für diese Arbeit wurde der Datensatz *olympics* von der Statsistik-Plattform *Kaggle* <https://www.kaggle.com> verwendet. Eine Kopie des Datensatzes ist unter <https://drive.google.com/open?id=1XsV3WVTb0EXjd9LhlpN2PkvUKJU8iY38> abrufbar.
...
```
```{r results='hide', message=FALSE, warning=FALSE}
insurance <- read_csv("olympics.csv")
```
...
Wie wir sehen können, haben wir es bei diesem Datensatz mit über 271k Observations zu tun.
...
Mit dem Befehl filter() bekommen wir von R genau definierte Zeilen aus dem zugrunde liegenden Datensatzes ausgegeben. Es gibt folgende Möglichkeiten zu filtern:
...
!= bedeutet "nicht". Wir sehen fast 3k weibliche Teilnehmer bisher an Olympia. 
...
Mh ein Mann mit 1,83m Körpergröße und nur 28kg Gewicht (bmi = 8,36) ... komisch.
...
Der letzte der 5 hier behandelten Befehle für Data Transformation ist summarize(). Er wird fast ausschließlich mit dem group_by() Befehl verwendet. Diese Kombination ermöglicht uns zu vergleichen.
...
Als Ergebnis sehen wir den durchschnittlichen BMI Wert von allen deutschen Olympiateilnehmer aufgeteilt nach Jahren.
