...
```{r}
world %>% filter(Region == "NORTHERN AFRICA") %>% select(Country, Birthrate, birthrate.transf)
```
...
Schiefe und Exzess lassen sich noch besser interpretieren, wenn man zusätzlich die Verteilung graphisch darstellt.
```{r}
mittelwert<- mean(world$Birthrate, na.rm = TRUE)
sd<- sd(world$Birthrate, na.rm = TRUE)
hist(world$Birthrate, freq = FALSE) + curve(dnorm(x, mittelwert, sd), add = TRUE)
```
