...
Die *map* Funktion transformiert einen Input durch die Anwendung einer Funktion auf jedes Element des Inputs und gibt anschließend einen Vektor aus, der dieselbe Länge besitzt wie die des Inputs. In unserem Beispiel wurde R den Befehl gegeben, jedes Element aus v (zuvor erstelltes Objekt) mit 10 zu multiplizieren. 
...
Mit *select* können gezielt Spalten ausgewählt werden, für die anschließend mit *map* die Kennzahlen angefordert werden sollen. 
...
Der *map* Befehl sieht hier anders aus als der verwendete beim t-Test. Ist aber nur eine andere Schreibweise. Hier die beiden Varianten gegenüber gesellt: 
