...
Häufig wird nicht nur ein Wert (zB. Mittelwert) für eine Variable benötigt, sondern die Werte für mehrere Spalten. Die dazugehörige Funktion lautet *colMeans*. Für die Auswahl mehrerer Variablen in R gibt es mehrere Möglichkeiten:
...
Es lassen sich auch via der Indexfunktion mit dem Zusatzargument *c()* mehrere Spalten geleichzeitig auswählen. Im nachfolgenden Beispiel werden die Spalten fünf und sechs dem Objekt „Auswahl3“ zugeordnet.
```{r}
...
Mit dem Befehl *glimpse* sehe wir, dass das neue Objekt „Auswahl5“ vier Variablen (von „Sex“ bis „Weight“) hat. 

```{r}
glimpse(Auswahl5)
```

#3 Change object'charactersitics

Objekte spielen eine zentrale Rolle bei der Arbeit mit R. Es gibt unterschiedlich Objekttypen: Beispielsweise Vektoren, Faktoren oder Data Frames. Numerische Variablen können in R sowohl als Vektoren als auch in Faktoren gespeichert werden. Wohingegen Vektoren ordinalskalierte oder metrische Daten enthalten, beinhalten Faktoren nominalskalierte Variablen. Werden nummerische Daten in R eingelesen, werden diese meiste als numerischen Vektor gespeichert, unabhängig vom tatsächlichen Skalenniveau. Es ist daher manchmal notwendig, die Objekteigenschaften zu konvertieren.  

##3.1 character to factores	
```{r}
class(olympics$Sex)
```
...
Vor dem Zuweisungspfeil steht der Name des neuen Faktors. Wenn der neue und alte Name identisch ist, wird der alte überschrieben. Wenn wir in beibehalten möchten, empfiehlt es sich (wie im Beispiel) ein neues, zusätzliches Objekt anzulegen. Betrachtet man den neuen Data Frame, sehen wir, dass die Variable `Sex` weiterhin als „M“ und „F“ ausgegeben wird. Es ist manchmal sinnvoll, Abkürzungen oder Zahlen in aussagekräftigere Begriffe umzuwandeln. Wir modifizieren daher den Befehl wie folgt: 
```{r}
...
Vor dem Zuweisungspfeil steht der Name der neuen Variable, hier „GebJahr“. Durch den Ausdruck olympics$ wird diese Variable automaisch den olympics-Daten hinzugefügt. 
```{r}
glimpse(olympics)
```
...
Und nun eine weiter mit dem Notendurchschnitt:
```{r}
...
Wir erkennen, dass die Werte von Summe_Noten und Notendurchschnitt gleich sind mit rowSums und rowMeans. 
...
Um eine Variable zu standardisieren, teilt man zusätzlich noch durch die Standardabweichung der Variablen. Die standardisierte Variable hat dann den Mittelwert 0 und die Standardabweichung 1. Die einzelnen Werte der standardisierten Werte nennt man dann z-Werte. Eine Standardisierung macht dann Sinn, wenn man sprichwörtlich Äpfel mit Birnen vergleichen möchte. So hat man beispielsweise bei einer Umfrage das Lebensalter von Kindern in einer Familie in Jahren erhoben. Bei weiteren Umfrage geschah das gleiche nur in Monaten und Wochen. Durch eine Standardisierung lassen sich die Kinder dennoch miteinander vergleichen ohne einzelne Werte umrechnen zu müssen. 
...
```{r}
which(olympics$Medal == "Gold")
```

Wichtig! Die ausgegebenen Werte beziehen sich auf die Zeilen, in denen die Daten der Person gespeichert sind. Man kann daher diese Ausgabe auch in der Index Funktion verwenden. Mit der Index Funktion werden gezielt bestimmte Zeilen und Spalten ausgewählt. Mit folgenden Befehl werden die Werte für die oben aufgeführte Zeilen ausgegeben. Natürlich handelt es sich bei jedem Wert um das Wort „Gold“, denn genau das war ja auch unsere Bedingung.
```{r}
olympics$Medal[which(olympics$Medal == "Gold")]
```

Nun kommt der Trick: Man kann nun jedes Wort durch ein beliebiges anders Wort oder beliebigen anderen Wert ersetzten, indem man hinter dem obigen Befehl den Zuweisungspfeil und dann das neue Wort/ den neuen Wert aufführt. Wir ersetzen nun das Wort „Gold“ durch „First_Place“:
```{r}
...
Wir haben nun mit zwei *which* Funktionen die Athleten in einer neuen Variablen `Altersgruppe` in zwei Untergruppen unterteilt. Die unter 25 jähren erhielten den Wert 0, die über 25 jähren den Wert 1. Verwendet man stattdessen die *ifelse* Funktion, kann man die gleiche Operation in nur einem Befehl ausführen lassen:
```{r}
olympics$Altersgruppe2 <- ifelse(olympics$Age >= 25, 1, 0)
```

Man sieht, die Aufteilung von 0 und 1 ist bei beiden Variablen („Altersgruppe1“ und „Altersgruppe2“ gleich. 
```{r}
glimpse(olympics$Altersgruppe1)
...
Wir können sehen, dass auch hier die Umkodierung gekappt hat, da die Variable `Note5.r2` und die Variable `Note5.r` gleiche Werte enthält. 
```{r}
