...
Um den Bezug zur Zeilensummer anzufordern, wird der Befehl um das Argument 1 erweitert.
```{r}
...
Das Ergebnis ist erstmal unübersichtlich, da die Spaltennamen zu lang ausgegeben werden. Wir benennen sie daher, während der data.frame Zuweisung um.
```{r}
Korrelation2 <- data.frame(Power = Automobile_data$Power, Price = Automobile_data$Car_Price, Weight = Automobile_data$`curb-weight`)
cor(Korrelation2)
```

Schon viel besser. Aber nun besteht das Problem mit den NA's noch. Natürlich könnte man mit der *na.omit* Funktion alle leere Spalten löschen aber dabei würde man evtl. wichtige Werte verlieren.  
...
Bei den Zusammenhangsmaßen in den Sozialwissenschaften gelten folgende Faustregeln:
...
Man kann ich auch für die Kovarinz mit einer Kombination von Variablen ausgeben lassen. 
```{r}
